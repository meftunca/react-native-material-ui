...
const Ripple = ({ disabled = false, hitSlop, pressRetentionOffset, children, rippleContainerBorderRadius = 5, testID, nativeID = Number(Math.random() * 100).toString(), accessible = false, onPressIn, onPressOut, delayPressIn = 150, delayPressOut = 150, delayLongPress = 360, accessibilityHint, accessibilityLabel, onPress, onLongPress, buttonRef, onLayout, onRippleAnimation = (animation, callback) => animation.start(callback), rippleColor = 'rgb(0, 0, 0)', rippleOpacity = 0.4, rippleDuration = 350, rippleSize = 0, rippleCentered = false, rippleSequential = false, rippleFades = true, ...props }) => {
    ...
    const setState = (newState) => setBaseState({ ...state, ...newState });
    useEffect(() => {
        setMounted(true);
        return () => {
            setMounted(false);
    ...
    const onLayoutFunc = (event) => {
        ...
        return (React.createElement(Animated.View, { pointerEvents: "none", style: [styles.ripple, rippleStyle], key: unique }));
    };
    let touchableProps = {
        delayLongPress,
        delayPressIn,
    ...
    return (React.createElement(View, { ref: buttonRef },
        React.createElement(TouchableWithoutFeedback, Object.assign({ accessibilityRole: "button" }, touchableProps),
