...
const Menu = ({
  ...
  const fade = useAnimation({
    ...
    duration,
    delay,
    easing: open === false ? Easing.bezier(0, 0.4, 0.7, 1) : Easing.bezier(1, 0.7, 0.4, 0)
  });
  useEffect(() => {
    if (triggerRef.current.measure) triggerRef.current.measure((ox, oy, width, height, px, py) => {
      setSize({ ...size,
        ox,
        oy,
        x: px,
  ...
  useEffect(() => {
    if (open === true) setModalVisible(true);else setTimeout(() => {
  ...
  let isTop = contentLayout.height + size.y >= height / 2,
      ...
      minWidthCalculate = size.width < (minWidth || contentLayout.width) ? minWidth || contentLayout.width : size.width;
  return React.createElement(React.Fragment, null, trigger && React.createElement(Ripple, {
    buttonRef: triggerRef,
    onPress: () => {
      setOpen(true);
    }
  }, trigger), React.createElement(Modal, {
    ...
    style: {
      ...
      transform: [{
        translateX: fade.interpolate({
          inputRange: [0, 1],
          outputRange: [(isLeft === false ? -1 : 1) * contentLayout.width + size.width, 0]
        })
      }, {
        translateY: fade.interpolate({
          inputRange: [0, 1],
          outputRange: [(isTop === false ? -1 : 1) * contentLayout.height + size.height, 0]
        })
      }],
...
Menu.propTypes = {
  children: _pt.node,
  trigger: _pt.element,
  render: _pt.oneOfType([_pt.any]),
  corderRadius: _pt.number,
  duration: _pt.number,
  delay: _pt.number,
  minWidth: _pt.number,
